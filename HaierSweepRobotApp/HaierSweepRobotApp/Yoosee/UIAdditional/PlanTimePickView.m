...
- (void)viewForCell:(NSUInteger)cell inScroll:(NSUInteger)scroll reusingCell:(UITableViewCell *)tc {
    ...
    switch (scroll) {
        ...
        case 0:{
            ...
            tc.textLabel.text=[NSString stringWithFormat:@"%i %@",cell,CustomLocalizedString(@"hour", nil)];
            break;
        }
        case 1:{
            
            tc.textLabel.text=[NSString stringWithFormat:@"%i %@",cell,CustomLocalizedString(@"minute", nil)];
            break;
        }
...
//选择当前cell
-(void)selectedCellFromPlanTimePickView:(NSUInteger)hourCell minCell:(NSUInteger)minCell{
    _date.hour = hourCell;
    [_picker selectCell:hourCell inScroll:0];
    
    _date.minute = minCell;
    [_picker selectCell:minCell inScroll:1];
}
