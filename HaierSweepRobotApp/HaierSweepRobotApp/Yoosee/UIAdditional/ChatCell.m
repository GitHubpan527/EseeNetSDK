...
-(void)layoutSubviews{
    ...
    if(!self.timeLabel){
        ...
        if(self.message.state==MESSAGE_STATE_SENDING){
            label.text = CustomLocalizedString(@"message_sending", nil);
        }else if(self.message.state==MESSAGE_STATE_SEND_FAILURE){
            label.text = CustomLocalizedString(@"message_send_failure", nil);
        }else{
        ...
        if(self.message.state==MESSAGE_STATE_SENDING){
            self.timeLabel.text = CustomLocalizedString(@"message_sending", nil);
        }else if(self.message.state==MESSAGE_STATE_SEND_FAILURE){
            self.timeLabel.text = CustomLocalizedString(@"message_send_failure", nil);
        }else{
    ...
    CGFloat nameLabelHeight = [Utils getStringHeightWithString:CustomLocalizedString(@"me", nil) font:XFontBold_14 maxWidth:self.headerView.frame.size.width];
    ...
    if(!self.nameLabel){
        ...
        if(isLeft){
            label.text = CustomLocalizedString(@"me", nil);
        }else{
            if(contact!=nil){
        ...
        if(isLeft){
            self.nameLabel.text = CustomLocalizedString(@"me", nil);
        }else{
            if(contact!=nil){
