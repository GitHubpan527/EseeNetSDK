    ...
    switch (ges.state) {
        ...
        case UIGestureRecognizerStateBegan:
        {
            ...
            UIActionSheet *sheet = [[UIActionSheet alloc] initWithTitle:nil
                                                 ...
                                                 destructiveButtonTitle:CustomLocalizedString(@"delete_photo", nil)
                                                      otherButtonTitles:CustomLocalizedString(@"save_to_photo", nil), nil];
            // Show the sheet
...
- (void)image: (UIImage *) image didFinishSavingWithError: (NSError *) error contextInfo: (void *) contextInfo{//保存图片到相册
    if(error != NULL){
        //[self makeToast:CustomLocalizedString(@"保存图片失败", nil)];
    }else{
        //[self makeToast:CustomLocalizedString(@"保存图片成功", nil)];
    }
}
