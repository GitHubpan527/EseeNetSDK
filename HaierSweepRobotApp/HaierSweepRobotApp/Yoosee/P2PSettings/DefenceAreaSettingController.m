        ...
        case RET_DEVICE_NOT_SUPPORT://不支持禁用、启用开关
        {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                if (!self.isNotRightPWD) {
                    //[self.view makeToast:CustomLocalizedString(@"device_not_support_sensor_switch", nil)];
                }
            ...
            if(result==0){
               ...
               [[P2PClient sharedClient] getDefenceAreaState:self.contact.contactId password:self.contact.contactPassword];
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    self.isLoadDefenceArea = NO;
                    [self.tableView reloadData];
                    NSString *promptString = [NSString stringWithFormat:@"%@:%@ %i %@",[self getDefenceGroupNameWithIndex:group],CustomLocalizedString(@"defence_item",nil),item+1,CustomLocalizedString(@"already_learn",nil)];
                    [self.view makeToast:promptString];
                });
            }else if(result==41){
                dispatch_async(dispatch_get_main_queue(), ^{
                    [self.view makeToast:CustomLocalizedString(@"device_not_support_defence_area", nil)];
                    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                        usleep(800000);
                        dispatch_async(dispatch_get_main_queue(), ^{
                            [self onBackPress];
...
-(NSString*)getDefenceGroupNameWithIndex:(NSInteger)index{
    switch (index) {
        case 0:
            return CustomLocalizedString(@"remote", nil);
            break;
        case 1:
            return CustomLocalizedString(@"hall", nil);
            break;
        case 2:
            return CustomLocalizedString(@"window", nil);
            break;
        case 3:
            return CustomLocalizedString(@"balcony", nil);
            break;
        case 4:
            return CustomLocalizedString(@"bedroom", nil);
            break;
        case 5:
            return CustomLocalizedString(@"kitchen", nil);
            break;
        case 6:
            return CustomLocalizedString(@"courtyard", nil);
            break;
        case 7:
            return CustomLocalizedString(@"door_lock", nil);
            break;
        case 8:
            return CustomLocalizedString(@"other", nil);
            break;
...
-(void)initDataSource{
    ...
    for (int i = 0; i < 9; i++) {
        ...
        switch (i) {
            case 0:
                [groupDic setObject:CustomLocalizedString(@"remote", nil) forKey:@"groupName"];
                break;
            case 1:
                [groupDic setObject:CustomLocalizedString(@"hall", nil) forKey:@"groupName"];
                break;
            case 2:
                [groupDic setObject:CustomLocalizedString(@"window", nil) forKey:@"groupName"];
                break;
            case 3:
                [groupDic setObject:CustomLocalizedString(@"balcony", nil) forKey:@"groupName"];
                break;
            case 4:
                [groupDic setObject:CustomLocalizedString(@"bedroom", nil) forKey:@"groupName"];
                break;
            case 5:
                [groupDic setObject:CustomLocalizedString(@"kitchen", nil) forKey:@"groupName"];
                break;
            case 6:
                [groupDic setObject:CustomLocalizedString(@"courtyard", nil) forKey:@"groupName"];
                break;
            case 7:
                [groupDic setObject:CustomLocalizedString(@"door_lock", nil) forKey:@"groupName"];
                break;
            case 8:
                [groupDic setObject:CustomLocalizedString(@"other", nil) forKey:@"groupName"];
                break;
            default:
                break;
        }
...
-(void)initComponent{
    ...
    self.navigationItem.title = CustomLocalizedString(@"defenceArea_set",nil);

    
    
    UIView *maskLayerView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, width, height)];
    
    UITableView *tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, width, height) style:UITableViewStylePlain];//UITableViewStyleGrouped,headView随cell一起滚动
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([statusData[row] intValue] == 1) {//开学习
        ...
        UIAlertView *learnAlert = [[UIAlertView alloc] initWithTitle:CustomLocalizedString(@"learn_defence_prompt", nil) message:@"" delegate:self cancelButtonTitle:CustomLocalizedString(@"cancel", nil) otherButtonTitles:CustomLocalizedString(@"ok", nil),nil];
        ...
        UIAlertView *clearAlert = [[UIAlertView alloc] initWithTitle:CustomLocalizedString(@"clear_defence_prompt", nil) message:@"" delegate:self cancelButtonTitle:CustomLocalizedString(@"cancel", nil) otherButtonTitles:CustomLocalizedString(@"ok", nil),nil];
