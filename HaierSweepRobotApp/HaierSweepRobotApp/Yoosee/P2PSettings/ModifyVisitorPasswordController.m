...
-(void)initComponent{
    ...
    self.navigationItem.title = CustomLocalizedString(@"modify_visitor_password", nil);
    ...
    field1.placeholder = CustomLocalizedString(@"input_new_visitor_password", nil);
    ...
    [clearButton setTitle:CustomLocalizedString(@"clear_visitor_pwd", nil) forState:UIControlStateNormal];
...
-(void)onSavePress{
    ...
    if(!newPassword||!newPassword.length>0){
        [self.view makeToast:CustomLocalizedString(@"input_new_visitor_password", nil)];
        return;
    }
    
    
    if([predicate evaluateWithObject:newPassword]==NO){
        [self.view makeToast:CustomLocalizedString(@"guest_password_number_format_error", nil)];
        return;
    }
    ...
    if(newPassword.length>9){
        [self.view makeToast:CustomLocalizedString(@"guest_password_too_long", nil)];
        return;
    }
...
-(void)onClearVisitorPassword{
    UIAlertView *unBindEmailAlert = [[UIAlertView alloc] initWithTitle:CustomLocalizedString(@"clear_visitor_pwd", nil) message:@"" delegate:self cancelButtonTitle:CustomLocalizedString(@"cancel", nil) otherButtonTitles:CustomLocalizedString(@"ok", nil),nil];
