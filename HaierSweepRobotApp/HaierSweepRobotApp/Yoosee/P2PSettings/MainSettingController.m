    ...
    switch(key){
        ...
        case RET_CHECK_DEVICE_UPDATE:
        {
            ...
            if(result==1){
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    NSString *title = [NSString stringWithFormat:@"%@:%@,%@:%@",CustomLocalizedString(@"cur_version_is", nil),curVersion,CustomLocalizedString(@"can_update_to", nil),upgVersion];
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    NSString *title = [NSString stringWithFormat:@"%@:%@,%@",CustomLocalizedString(@"cur_version_is", nil),curVersion,CustomLocalizedString(@"can_update_sd", nil)];
                    UIAlertView *deleteAlert = [[UIAlertView alloc] initWithTitle:title message:@"" delegate:self cancelButtonTitle:CustomLocalizedString(@"cancel", nil) otherButtonTitles:CustomLocalizedString(@"ok", nil),nil];
                    deleteAlert.tag = ALERT_TAG_UPDATE;
                    [deleteAlert show];
                    [deleteAlert release];
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    [self.view makeToast:[NSString stringWithFormat:@"%@:%@",CustomLocalizedString(@"now_version_is_latest", nil),curVersion]];
                });
            }else if(result==58){
                dispatch_async(dispatch_get_main_queue(), ^{
                    
                    [self.view makeToast:CustomLocalizedString(@"other_was_check_device_update", nil)];
                });
            }else{
                dispatch_async(dispatch_get_main_queue(), ^{
        ...
        case RET_SWITCH_APSTA_MODE:
        {
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                if (result == 0)
                {
                    [self.view makeToast:CustomLocalizedString(@"apsta_switch_ok", nil)];
                }
                else
                {
...
-(void)initComponent{
    ...
    if(self.contact.contactType==CONTACT_TYPE_IPC || self.contact.contactType==CONTACT_TYPE_DOORBELL || self.contact.contactId.intValue<256){//IP添加设备
        ...
        viewDeviceInfoButton.frame = CGRectMake(ScreenWidth-100, 20, 80, 40);
        ...
        UIImageView *arrowImageV = [[UIImageView alloc] initWithFrame:CGRectMake(ScreenWidth-23, 32.5, 8, 15)];
        arrowImageV.image = [UIImage imageNamed:@"箭头"];
        [topInfoBarView addSubview:arrowImageV];
...
//        deviceInfoLabel.text = CustomLocalizedString(@"device_info", nil);
...
//        [deviceInfoLabel release];
    ...
    UITableView *tableView = [[UITableView alloc] initWithFrame:CGRectMake(0,TOP_INFO_BAR_HEIGHT, width, height-(TOP_INFO_BAR_HEIGHT)) style:UITableViewStyleGrouped];
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
        ...
        case 0:
        {
            if ([[AppDelegate sharedDefault] dwApContactID] == 0) {
                if(row==0){
                    ...
                    [cell setLabelText:CustomLocalizedString(@"time_set", nil)];
                    ...
                    [cell setLabelText:CustomLocalizedString(@"media_set", nil)];
                }else if(row==2){
                    backImg = [UIImage imageNamed:@"bg_bar_btn_center.png"];
                    backImg_p = [UIImage imageNamed:@"bg_bar_btn_center_p.png"];
                    [cell setLeftIcon:@"ic_ctl_security.png"];
                    [cell setLabelText:CustomLocalizedString(@"security_set", nil)];
                }else if(row==3){
                    backImg = [UIImage imageNamed:@"bg_bar_btn_center.png"];
                    backImg_p = [UIImage imageNamed:@"bg_bar_btn_center_p.png"];
                    [cell setLeftIcon:@"ic_ctl_net.png"];
                    [cell setLabelText:CustomLocalizedString(@"network_set", nil)];
                }else if(row==4){
                    backImg = [UIImage imageNamed:@"bg_bar_btn_center.png"];
                    backImg_p = [UIImage imageNamed:@"bg_bar_btn_center_p.png"];
                    [cell setLeftIcon:@"ic_ctl_alarm.png"];
                    [cell setLabelText:CustomLocalizedString(@"alarm_set", nil)];
                }else if(row==5){
                    backImg = [UIImage imageNamed:@"bg_bar_btn_center.png"];
                    backImg_p = [UIImage imageNamed:@"bg_bar_btn_center_p.png"];
                    [cell setLeftIcon:@"ic_ctl_record.png"];
                    [cell setLabelText:CustomLocalizedString(@"record_set", nil)];
                }else if(row==6){
                    backImg = [UIImage imageNamed:@"bg_bar_btn_center.png"];
                    backImg_p = [UIImage imageNamed:@"bg_bar_btn_center_p.png"];
                    [cell setLeftIcon:@"ic_ctl_defence_area.png"];
                    [cell setLabelText:CustomLocalizedString(@"defenceArea_set", nil)];
                    ...
                    [cell setLabelText:CustomLocalizedString(@"push_language", nil)];
                }
                    ...
                    [cell setLabelText:CustomLocalizedString(@"device_update", nil)];
                ...
                else if(row == _iApStaSwitchRow)//模式切换
                {
                    ...
                    [cell setLabelText:CustomLocalizedString(@"apsta_switch_to_ap", nil)];
                }
            }
            else
            {
                if(row==0){
                    ...
                    [cell setLabelText:CustomLocalizedString(@"ap_mode_set_password", nil)];
                }
                else if(row == 6)//模式切换
                {
                    backImg = [UIImage imageNamed:@"bg_bar_btn_bottom.png"];
                    backImg_p = [UIImage imageNamed:@"bg_bar_btn_bottom_p.png"];
                    [cell setLeftIcon:@"ic_ctl_modeswitch.png"];
                    [cell setLabelText:CustomLocalizedString(@"apsta_switch_to_sta", nil)];
                }
            }
    ...
    */
    ...
    LjpSetupCell *cell = nil;
    if(indexPath.row == _iDevUpdateRow){
        cell = [tableView dequeueReusableCellWithIdentifier:identifier2];
        if(cell==nil){
            cell = [[NSBundle mainBundle] loadNibNamed:@"LjpSetupCell" owner:self options:nil].firstObject;
            //[cell setBackgroundColor:XBGAlpha];
        }
        ...
        case 0:
        {
            if ([[AppDelegate sharedDefault] dwApContactID] == 0) {
                if(row==0){
                    ...
                    iconImageV.image = LCImage(@"安全-");
                    titleLabel.text = CustomLocalizedString(@"security_set", nil);
                }else if(row==3){
                    iconImageV.image = LCImage(@"网络-");
                    titleLabel.text = CustomLocalizedString(@"network_set", nil);
                }else if(row==4){
                    iconImageV.image = LCImage(@"报警-");
                    titleLabel.text = CustomLocalizedString(@"alarm_set", nil);
                }else if(row==5){
                    iconImageV.image = LCImage(@"录像-");
                    titleLabel.text = CustomLocalizedString(@"record_set", nil);
                }
//                else if(row==6){
//                    iconImageV.image = LCImage(@"防区-");
//                    titleLabel.text = CustomLocalizedString(@"defenceArea_set", nil);
//                }else if(row==7){//推送语言设置
//                    iconImageV.image = LCImage(@"设备-");
//                    titleLabel.text = CustomLocalizedString(@"push_language", nil);
//                }
//                else if(row==_iDevUpdateRow){//设备更新
//                    iconImageV.image = LCImage(@"检查更新-");
//                    titleLabel.text = CustomLocalizedString(@"device_update", nil);
//                }
//                else if(row == _iApStaSwitchRow)//模式切换
//                {
//                    iconImageV.image = LCImage(@"ic_ctl_modeswitch.png");
//                    titleLabel.text = CustomLocalizedString(@"apsta_switch_to_ap", nil);
//                }
            }
            else
            {
    ...
    switch(section){
        case 0:
        {
            if ([[AppDelegate sharedDefault] dwApContactID] == 0) {
                ...
                {
                    UIAlertView *deleteAlert = [[UIAlertView alloc] initWithTitle:CustomLocalizedString(@"apsta_switch_sure", nil)
                                                                          message:@""
                                                                         delegate:self
                                                                cancelButtonTitle:CustomLocalizedString(@"cancel", nil)
                                                                otherButtonTitles:CustomLocalizedString(@"ok", nil),nil];
...
-(void)showDeviceInfoViewWithCurVersion:(NSString*)curVersion kernelVersion:(NSString*)kernelVersion rootfsVersion:(NSString*)rootfsVersion ubootVersion:(NSString*)ubootVersion{
    ...
    parent.tag = 800;
    parent.frame = CGRectMake(0, 0, self.view.frame.size.width, ScreenHeight);
    ...
    titleLabel.font = XFontBold_16;
    titleLabel.text = CustomLocalizedString(@"device_info", nil);
    ...
    for(int i=0;i<8;i++){
        ...
        switch (i) {
            case 0:
                label.text = CustomLocalizedString(@"cur_version", nil);
                break;
            case 1:
                label.text = curVersion;
                break;
            case 2:
                label.text = CustomLocalizedString(@"kernel_version", nil);
                break;
            case 3:
                label.text = kernelVersion;
                break;
            case 4:
                label.text = CustomLocalizedString(@"rootfs_version", nil);
                break;
            case 5:
                label.text = rootfsVersion;
                break;
            case 6:
                label.text = CustomLocalizedString(@"uboot_version", nil);
                break;
            case 7:
    ...
    [self.navigationController.view addSubview:parent];
    parent.alpha = 0.3;
...
-(void)hideDeviceInfoView:(UIButton*)button{
    ...
    UIButton *parent = (UIButton*)[self.navigationController.view viewWithTag:800];
