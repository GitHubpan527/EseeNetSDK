...
-(void)initComponent{
    ...
    self.navigationItem.title = CustomLocalizedString(@"network_set",nil);
    ...
    switch (section) {
        case 0:
        {
            if(row==0){
                ...
                [emailCell setLeftLabelText:CustomLocalizedString(@"net_type", nil)];
        ...
        case 1:
        {
            if(row==0){
                ...
                [emailCell setLeftLabelText:CustomLocalizedString(@"wifi_list", nil)];
                if(self.isLoadingWifiList){
                    [emailCell setLeftIconHidden:YES];
                    [emailCell setLeftLabelHidden:NO];
                    [emailCell setRightIconHidden:YES];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==1&&indexPath.row>0&&(indexPath.row-1)!=self.currentWifiIndex){
        ...
        UIAlertView *promptAlert = [[UIAlertView alloc] initWithTitle:CustomLocalizedString(@"change_net_prompt", nil) message:@"" delegate:self cancelButtonTitle:CustomLocalizedString(@"cancel", nil) otherButtonTitles:CustomLocalizedString(@"ok", nil),nil];
    ...
    switch(alertView.tag){
        ...
        case ALERT_TAG_CHANGE_WIFI:
        {
            if(buttonIndex==0){
                ...
                NSString *name = [self.names objectAtIndex:self.selectWifiIndex];
                UIAlertView *inputAlert = [[UIAlertView alloc] initWithTitle:CustomLocalizedString(@"input_wifi_password", nil) message:name delegate:self cancelButtonTitle:CustomLocalizedString(@"cancel", nil) otherButtonTitles:CustomLocalizedString(@"ok", nil), nil];
            ...
            if(buttonIndex==0){
                ...
                if (type == 0) {//nopwd wifi
                    ...
                    if(!inputPwd||inputPwd.length==0){
                        [self.view makeToast:CustomLocalizedString(@"input_wifi_password", nil)];
                        return;
                    }
                    
                    if(inputPwd.length<8){
                        [self.view makeToast:CustomLocalizedString(@"wifi_password_format_error", nil)];
                        return;
                    }
                }
