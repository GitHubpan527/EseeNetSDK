...
-(void)initComponent{
    ...
    self.navigationItem.title = CustomLocalizedString(@"bind_account",nil);
    ...
    field1.placeholder = CustomLocalizedString(@"input_alarm_id", nil);
...
-(void)onSavePress{
    ...
    if(!alarmId||!alarmId.length>0){
        [self.view makeToast:CustomLocalizedString(@"input_alarm_id", nil)];
        return;
    }
    
    if([alarmId characterAtIndex:0]!='0'){
        [self.view makeToast:CustomLocalizedString(@"alarm_id_must_zero_begin", nil)];
        return;
    }
