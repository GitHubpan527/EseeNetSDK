...
-(void)viewDidAppear:(BOOL)animated{
    ...
    if(!self.countryCode||!self.countryCode.length>0){
        ...
        if([language hasPrefix:@"zh"]){
            self.countryCode = @"86";
            self.countryName = CustomLocalizedString(@"china", nil);
        }else{
            self.countryCode = @"1";
            self.countryName = CustomLocalizedString(@"america", nil);
        }
    ...
    if(self.isSessionIdError){
        ...
        [self.view makeToast:CustomLocalizedString(@"session_error", nil) duration:2.0 position:@"center"];
    ...
    if(self.isP2PVerifyCodeError){
        ...
        NSString *codeError = [NSString stringWithFormat:@"%@(46)",CustomLocalizedString(@"id_internal_error", nil)];
...
-(void)initComponent{
    ...
    [topBar setTitle:CustomLocalizedString(@"account_login",nil)];
    ...
    UISegmentedControl *segment = [[UISegmentedControl alloc] initWithItems:@[CustomLocalizedString(@"email_id_login", nil),CustomLocalizedString(@"phone_login", nil)]];
    ...
    field1.placeholder = CustomLocalizedString(@"input_username", nil);
    ...
    field2.placeholder = CustomLocalizedString(@"input_password", nil);
    ...
    [loginBtn setTitle:CustomLocalizedString(@"login", nil) forState:UIControlStateNormal];
    ...
    [registerBtn setTitle:CustomLocalizedString(@"new_account_register", nil) forState:UIControlStateNormal];
    ...
    //forget password
    CGFloat forgetLabelWidth1 = [Utils getStringWidthWithString:CustomLocalizedString(@"forget_password", nil) font:XFontBold_14 maxWidth:width];
    ...
    forgetLabel1.text = CustomLocalizedString(@"forget_password", nil);
    ...
    field3.placeholder = CustomLocalizedString(@"input_phone", nil);
    ...
    field4.placeholder = CustomLocalizedString(@"input_password", nil);
    ...
    [loginBtn2 setTitle:CustomLocalizedString(@"login", nil) forState:UIControlStateNormal];
    ...
    [registerBtn2 setTitle:CustomLocalizedString(@"new_account_register", nil) forState:UIControlStateNormal];
    ...
    //forget password
    CGFloat forgetLabelWidth2 = [Utils getStringWidthWithString:CustomLocalizedString(@"forget_password", nil) font:XFontBold_14 maxWidth:width];
    ...
    forgetLabel2.text = CustomLocalizedString(@"forget_password", nil);
    ...
    //文字宽、高
    CGFloat labelPrompt_w = [Utils getStringWidthWithString:CustomLocalizedString(@"un_rem_pass", nil) font:XFontBold_16 maxWidth:width];
    CGFloat labelPrompt_h = [Utils getStringHeightWithString:CustomLocalizedString(@"un_rem_pass", nil) font:XFontBold_16 maxWidth:width];
...
-(void)showRememberPwdPrompt:(BOOL)isRememberPwd{//记住用户的登录密码
    ...
    if (isRememberPwd) {
        ...
        labelPrompt.text = CustomLocalizedString(@"rem_pass", nil);
    }else{
        imageViewPrompt.image = [UIImage imageNamed:@"ic_unremember_pwd.png"];
        labelPrompt.text = CustomLocalizedString(@"un_rem_pass", nil);
    }
...
-(void)onProgressAlertExit{
    sleep(1.5);
    [self.view makeToast:CustomLocalizedString(@"user_unexist", nil)];
}
...
-(void)onLoginPress:(id)sender{
    ...
    if(!username||!username.length>0){
        ...
        [self.view makeToast:CustomLocalizedString(@"unInputUsername", nil)];
        return;
    }
    ...
    if(!password||!password.length>0){
        ...
        [self.view makeToast:CustomLocalizedString(@"unInputPassword", nil)];
        return;
    }
    ...
    [[NetManager sharedManager] loginWithUserName:username password:password token:[AppDelegate sharedDefault].token callBack:^(id result){
        ...
        switch(loginResult.error_code){
            ...
            default:
            {
                ...
                [self.view makeToast:[NSString stringWithFormat:@"%@:%i",CustomLocalizedString(@"login_failure", nil),loginResult.error_code]];
            }
                break;
        }
        ...
        DLog(@"%@",username);
        [[NetManager sharedManager] loginWithUserName:@"+86-13522863358" password:@"123456" token:[AppDelegate sharedDefault].token callBack:^(id result){
            ...
            switch(loginResult.error_code){
                ...
                {
                    [self.view makeToast:CustomLocalizedString(@"login_failure", nil)];
                }
                    break;
                case NET_RET_SYSTEM_MAINTENANCE_ERROR:
                {
