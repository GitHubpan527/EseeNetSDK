...
-(void)viewWillDisappear:(BOOL)animated{
    ...
    [AppDelegate sharedDefault].isBeingInP2PVideo = NO;
}
...
- (void)viewDidLoad
{
    ...
    [AppDelegate sharedDefault].isBeingInP2PVideo = YES;//当前是视频通话或呼叫状态下
...
-(void)initComponent{
    ...
    if(p2pCallType==P2PCALL_TYPE_MONITOR){
        ...
        self.navigationItem.title = CustomLocalizedString(@"call",nil);
    }
    ...
    rejectLabel.text = CustomLocalizedString(@"reject", nil);
    ...
    acceptLabel.text = CustomLocalizedString(@"accept", nil);
    ...
    UIView *mainView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, width, acceptBtn.frame.origin.y)];
    ...
    if(p2pCallType==P2PCALL_TYPE_VIDEO){
        if(isBCalled){
            headerLabel.text = [NSString stringWithFormat:@"%@%@",CustomLocalizedString(@"call_in_prompt", nil),callId];
        }else{
            headerLabel.text = [NSString stringWithFormat:@"%@%@",CustomLocalizedString(@"call_out_prompt", nil),callId];
        }
    }else{
//        headerLabel.text = [NSString stringWithFormat:@"%@%@",CustomLocalizedString(@"monitor_out_prompt", nil),callId];
        headerLabel.text = CustomLocalizedString(@"monitor_out_prompt", nil);
    }
