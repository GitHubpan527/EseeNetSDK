...
-(void)initComponent{
    ...
    CGFloat width = rect.size.width-64;
    
    
    self.navigationItem.title = CustomLocalizedString(@"ap_mode_set_password", nil);
    ...
    [saveButton setTitle:CustomLocalizedString(@"control", nil) forState:UIControlStateNormal];
    ...
    UILabel *lableID = [[UILabel alloc] initWithFrame:CGRectMake(BAR_BUTTON_MARGIN_LEFT_AND_RIGHT, 20+80, width-BAR_BUTTON_MARGIN_LEFT_AND_RIGHT*2, TEXT_FIELD_HEIGHT)];
    lableID.text = [NSString stringWithFormat:@"%@:  %@", CustomLocalizedString(@"ap_device_id", nil), contactid];
    ...
    field2.placeholder = CustomLocalizedString(@"ap_password_length_tip", nil);
...
//    field2.secureTextEntry = YES;
    ...
    lableTip.numberOfLines = 0;    lableTip.text = CustomLocalizedString(@"ap_reconnect_wifi_tip", nil);
...
-(void)onSavePress{
    ...
    if(password.length>16 || password.length<8){
        [self.view makeToast:CustomLocalizedString(@"ap_password_length_tip", nil)];
        return;
    }
    ...
    {
        NSLog(@"set ok");
        UIAlertView *tip = [[UIAlertView alloc] initWithTitle:CustomLocalizedString(@"ap_setwifiok_tip", nil)
                                                      message:@""
                                                     delegate:self
                                            cancelButtonTitle:CustomLocalizedString(@"ok",nil)
