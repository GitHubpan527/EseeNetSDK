        ...
        case RET_GET_PLAYBACK_FILES:
        {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                if (self.playbackFiles.count < 1) {
                    [self.view makeToast:CustomLocalizedString(@"no_playback_file", nil)];
                }
            });
...
-(void)initComponent{
    ...
    self.navigationItem.title = CustomLocalizedString(@"playback",nil);
    ...
    UIView *searchBarView = [[UIView alloc] initWithFrame:CGRectMake(0, TOP_INFO_BAR_HEIGHT, width, SEARCH_BAR_HEIGHT)];
    ...
    for(int i=0;i<4;i++){
        ...
        if(i==0){
            ...
            textLabel.text = CustomLocalizedString(@"recent_one_day", nil);
        }else if(i==1){
            textLabel.textColor = UIColorFromRGB(0x808080);
            textLabel.text = CustomLocalizedString(@"recent_three_day", nil);
        }else if(i==2){
            textLabel.textColor = UIColorFromRGB(0x808080);
            textLabel.text = CustomLocalizedString(@"recent_one_month", nil);
        }else if(i==3){
            textLabel.textColor = UIColorFromRGB(0x808080);
            textLabel.text = CustomLocalizedString(@"custom", nil);
        }
    ...
    UITableView *tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, TOP_INFO_BAR_HEIGHT+SEARCH_BAR_HEIGHT, width, height-(TOP_INFO_BAR_HEIGHT+SEARCH_BAR_HEIGHT)) style:UITableViewStylePlain];
    ...
    UIView *searchMaskView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.height)];
    ...
    UIView *movieView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.height)];
...
-(void)initCustomView{
    ...
    searchLabel.text = CustomLocalizedString(@"search", nil);
    ...
    startLeftLabel.text = CustomLocalizedString(@"start_time", nil);
    ...
    endLeftLabel.text = CustomLocalizedString(@"end_time", nil);
...
//    [datePicker release];
    ...
    NSArray * arr = @[CustomLocalizedString(@"year", nil),CustomLocalizedString(@"month", nil),CustomLocalizedString(@"day", nil),CustomLocalizedString(@"hour", nil),CustomLocalizedString(@"minute", nil)];
...
-(void)onCustomSearch:(UIButton*)button{
    ...
    if(!startTime||!startTime.length>0){
        ...
        [self.view makeToast:CustomLocalizedString(@"unselected_start_time", nil)];
        return;
    }
    
    if(!endTime||!endTime.length>0){
        [self.view makeToast:CustomLocalizedString(@"unselected_end_time", nil)];
        return;
    }
    ...
    if([startDate timeIntervalSince1970]>=[endDate timeIntervalSince1970]){
        [self.view makeToast:CustomLocalizedString(@"start_time_must_before_end_time", nil)];
        return;
    }
