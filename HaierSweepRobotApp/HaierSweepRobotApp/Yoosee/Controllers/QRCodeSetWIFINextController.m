...
-(void)initComponent{
    ...
    self.navigationItem.title = CustomLocalizedString(@"qrcode",nil);
    ...
    UIView *smartKeyPromptView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, width, height)];
    ...
    UIImageView *helpContent = [[UIImageView alloc] initWithFrame:CGRectMake((smartKeyPromptView.frame.size.width-WAITING_CONTENT_VIEW_WIDTH)/2, (smartKeyPromptView.frame.size.height-WAITING_CONTENT_VIEW_HEIGHT)/2, WAITING_CONTENT_VIEW_WIDTH, WAITING_CONTENT_VIEW_HEIGHT)];
    ...
    waitingContentLabel.text = CustomLocalizedString(@"waiting_content_prompt", nil);
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        if(!self.isFinish){
            dispatch_async(dispatch_get_main_queue(), ^{
                [self.smartKeyPromptView setHidden:YES];
                UIAlertView *alert = [[UIAlertView alloc] initWithTitle:CustomLocalizedString(@"set_wifi_failed_title", nil) message:CustomLocalizedString(@"set_wifi_failed_prompt", nil) delegate:self cancelButtonTitle:CustomLocalizedString(@"ok", nil) otherButtonTitles:nil,nil];
