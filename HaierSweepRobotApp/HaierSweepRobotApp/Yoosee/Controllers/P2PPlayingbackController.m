...
-(void)initComponent{
    ...
    //ljp
    backView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, width, 50)];
    [backView setBackgroundColor:[UIColor colorWithRed:0 green:0 blue:0 alpha:144.0/255.0]];
    ...
    for(int i=0;i<5;i++){
        ...
        UIImageView *iconView = [[UIImageView alloc] initWithFrame:CGRectMake(button.frame.size.width/2.0-20,button.frame.size.height/2.0-12.5,40,25)];
        switch(i){
            case 0:
            {
            ...
            case 4:
            {
                iconView.image = [UIImage imageNamed:@"ic_ctl_hungup修改.png"];
            }
                break;
        }
    ...
    switch(button.tag){
        ...
        case 1:
        {
            if([[P2PClient sharedClient] getPlaybackCurrentFileIndex]==0){
                [self.view makeToast:CustomLocalizedString(@"no_previous_files", nil)];
                return;
            }
        ...
        case 3:
        {
            if([[P2PClient sharedClient] getPlaybackCurrentFileIndex]>=([[P2PClient sharedClient] getPlaybackFilesLength]-1)){
                [self.view makeToast:CustomLocalizedString(@"no_next_files", nil)];
            }
...
-(void)onSingleTap{
    ...
    if (self.isShowControllerBar) {
        ...
        [UIView animateWithDuration:0.2 animations:^{
            backView.frame = CGRectMake(0, -50, self.view.frame.size.width, 50);
        }];
    }else{
        self.isShowControllerBar = !self.isShowControllerBar;
        ...
        [UIView animateWithDuration:0.2 animations:^{
            backView.frame = CGRectMake(0, 0, self.view.frame.size.width, 50);
        }];
    }
}
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                tempDelegate.mainVC.isShowP2PView = NO;
