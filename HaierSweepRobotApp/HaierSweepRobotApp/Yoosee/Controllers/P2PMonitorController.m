...
-(void)viewWillAppear:(BOOL)animated{
    ...
    [[P2PClient sharedClient] setCallId:self.monitorContact.contactId];
    [[P2PClient sharedClient] setP2pCallType:P2PCALL_TYPE_MONITOR];
    [[P2PClient sharedClient] setCallPassword:self.monitorContact.contactPassword];
    //rtsp监控界面弹出修改
    [self monitorP2PCall];
        ...
        case RET_SET_GPIO_CTL:
        {
            ...
            if (result == 0) {
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    [self.view makeToast:CustomLocalizedString(@"not_open", nil)];
                });
            }else if (result == MESG_GPIO_CTRL_QUEUE_IS_FULL){
                //操作过于频繁，之前的操作未执行完
                dispatch_async(dispatch_get_main_queue(), ^{
                    
                    [self.view makeToast:CustomLocalizedString(@"too_frequent", nil)];
                });
            }else if(result == MESG_SET_DEVICE_NOT_SUPPORT){
                //设备不支持此操作
                dispatch_async(dispatch_get_main_queue(), ^{
                    
                    [self.view makeToast:CustomLocalizedString(@"not_support_operation", nil)];
                });
            }
        }
            break;
...
- (void)viewDidLoad
{
    ...
    self.navigationItem.leftBarButtonItem = [UIBarButtonItem lc_itemWithIcon:@"返回按钮" block:^{
        [self backClick];
    }];
    ...
    mainVC.mainControllerDelegate = self;
...
-(void)hiddenMonitoringUI:(BOOL)isHidden callErrorInfo:(NSDictionary*)info isReCall:(BOOL)isReCall{
    if (isHidden) {
        ...
        if (isReCall) {
            ...
            self.labelTip.text = [NSString stringWithFormat:@"%@",CustomLocalizedString(@"monitor_out_prompt", nil)];
...
-(NSString *)getCallErrorStringWith:(int)errorFlag{
    switch(errorFlag)
    {
        ...
        {
            return CustomLocalizedString(@"id_unknown_error", nil);
            
        }
            break;
        case CALL_ERROR_DESID_NOT_ENABLE:
        {
            return CustomLocalizedString(@"id_disabled", nil);
        }
            break;
        case CALL_ERROR_DESID_OVERDATE:
        {
            return CustomLocalizedString(@"id_overdate", nil);
        }
            break;
        case CALL_ERROR_DESID_NOT_ACTIVE:
        {
            return CustomLocalizedString(@"id_inactived", nil);
        }
            break;
        case CALL_ERROR_DESID_OFFLINE:
        {
            return CustomLocalizedString(@"id_offline", nil);
        }
            break;
        case CALL_ERROR_DESID_BUSY:
        {
            return CustomLocalizedString(@"id_busy", nil);
        }
            break;
        case CALL_ERROR_DESID_POWERDOWN:
        {
            return CustomLocalizedString(@"id_powerdown", nil);
        }
            break;
        case CALL_ERROR_NO_HELPER:
        {
            return CustomLocalizedString(@"id_connect_failed", nil);
        }
            break;
        case CALL_ERROR_HANGUP:
        {
            return CustomLocalizedString(@"id_hangup", nil);
            
            break;
        }
        case CALL_ERROR_TIMEOUT:
        {
            return CustomLocalizedString(@"id_timeout", nil);
        }
            break;
        case CALL_ERROR_INTER_ERROR:
        {
            return CustomLocalizedString(@"id_internal_error", nil);
        }
            break;
        case CALL_ERROR_RING_TIMEOUT:
        {
            return CustomLocalizedString(@"id_no_accept", nil);
        }
            break;
        case CALL_ERROR_PW_WRONG:
        {
            return CustomLocalizedString(@"id_password_error", nil);
        }
            break;
        ...
        {
            return CustomLocalizedString(@"id_not_support", nil);
        }
            break;
        default:
        {
...
-(void)initComponentForHorizontalScreen{
    ...
    for(int i=0;i<rightItemCount;i++){
        ...
        if(rightItemCount==2){//NPC
            if(i==0){
                label.text = CustomLocalizedString(@"SD", nil);
                label.tag = CONTROLLER_LABEL_TAG_SD;
                button.tag = CONTROLLER_BTN_TAG_SD;
            }else if(i==1){
                label.text = CustomLocalizedString(@"LD", nil);
            ...
            if(i==0){
                label.text = CustomLocalizedString(@"HD", nil);
    ...
    if (rightItemCount == 2) {
        [resolutionButton setTitle:CustomLocalizedString(@"LD", nil) forState:UIControlStateNormal];
    }else{
        [resolutionButton setTitle:CustomLocalizedString(@"SD", nil) forState:UIControlStateNormal];
    }
...
-(void)initComponentForPortrait{
    ...
    //文字frame
    NSString *labelTipText = [NSString stringWithFormat:@"%@",CustomLocalizedString(@"玩命加载中...", nil)];
    ...
    labelTip.text = [NSString stringWithFormat:@"%@",CustomLocalizedString(@"monitor_out_prompt", nil)];
    ...
    //进入横屏时，调整frame
    //退出横屏时，也调整frame
    UIView *pressView = [[UIView alloc] initWithFrame:CGRectMake(10, self.canvasframe.size.height-PRESS_LAYOUT_WIDTH_AND_HEIGHT, PRESS_LAYOUT_WIDTH_AND_HEIGHT/2, PRESS_LAYOUT_WIDTH_AND_HEIGHT)];
    ...
    */
    ...
    //设置
    self.navigationItem.rightBarButtonItem = [UIBarButtonItem lc_itemWithIcon:@"摄像头设置-" block:^{
        MainSettingController *mainSettingController = [[MainSettingController alloc] init];
        mainSettingController.contact = self.monitorContact;
        [self.navigationController pushViewController:mainSettingController animated:YES];
        [mainSettingController release];
    }];
    
    //声音、横屏工具栏
    UIView *midToolHView = [[UIView alloc] initWithFrame:CGRectMake(0.0, CGRectGetMaxY(self.canvasView.frame), width, 50)];
    midToolHView.backgroundColor = XWhite;
    [self.view addSubview:midToolHView];
    self.midToolHView = midToolHView;//全屏时，隐藏
    [midToolHView release];
...
//    [switchScreenImageViewH release];
    ...
    //
    LCMonitorButton *button1 = [LCMonitorButton buttonWithType:UIButtonTypeCustom];
    button1.frame = CGRectMake(ScreenWidth/4.0*0+ScreenWidth/8.0-20, 5, 40, 40);
    //button1.tag = kOperatorBtnTag_Playback;
    [button1 setImage:[UIImage imageNamed:@"相册-"] forState:UIControlStateNormal];
    [button1 addTarget:self action:@selector(lookImageAction:) forControlEvents:UIControlEventTouchUpInside];
    button1.layer.masksToBounds = YES;
    button1.layer.cornerRadius = 20;
    button1.backgroundColor = [UIColor lc_colorWithR:235 G:244 B:254 alpha:1.0];
    [self.midToolHView addSubview:button1];
    
    //
    LCMonitorButton *button2 = [LCMonitorButton buttonWithType:UIButtonTypeCustom];
    button2.frame = CGRectMake(ScreenWidth/4.0*1+ScreenWidth/8.0-20, 5, 40, 40);
    button2.tag = kOperatorBtnTag_Playback;
    [button2 setImage:[UIImage imageNamed:@"摄像头-"] forState:UIControlStateNormal];
    [button2 addTarget:self action:@selector(contactButtonAction:) forControlEvents:UIControlEventTouchUpInside];
    button2.layer.masksToBounds = YES;
    button2.layer.cornerRadius = 20;
    button2.backgroundColor = [UIColor lc_colorWithR:235 G:244 B:254 alpha:1.0];
    [self.midToolHView addSubview:button2];
    
    //
    LCMonitorButton *button3 = [LCMonitorButton buttonWithType:UIButtonTypeCustom];
    button3.frame = CGRectMake(ScreenWidth/4.0*2+ScreenWidth/8.0-20, 5, 40, 40);
    button3.tag = SOUND_BUTTON_H_TAG;
    [button3 setImage:[UIImage imageNamed:@"喇叭-"] forState:UIControlStateNormal];
    [button3 addTarget:self action:@selector(onVerticalBtnPress:) forControlEvents:UIControlEventTouchUpInside];
    button3.layer.masksToBounds = YES;
    button3.layer.cornerRadius = 20;
    button3.backgroundColor = [UIColor lc_colorWithR:235 G:244 B:254 alpha:1.0];
    [self.midToolHView addSubview:button3];
    
    //
    LCMonitorButton *button4 = [LCMonitorButton buttonWithType:UIButtonTypeCustom];
    button4.frame = CGRectMake(ScreenWidth/4.0*3+ScreenWidth/8.0-20, 5, 40, 40);
    button4.tag = SWITCH_SCREEN_BUTTON_H_TAG;
    [button4 setImage:[UIImage imageNamed:@"展开-"] forState:UIControlStateNormal];
    [button4 addTarget:self action:@selector(onVerticalBtnPress:) forControlEvents:UIControlEventTouchUpInside];
    button4.layer.masksToBounds = YES;
    button4.layer.cornerRadius = 20;
    button4.backgroundColor = [UIColor lc_colorWithR:235 G:244 B:254 alpha:1.0];
    [self.midToolHView addSubview:button4];

    
    //
    /*
    LCMonitorButton *button4 = [LCMonitorButton buttonWithType:UIButtonTypeCustom];
    button4.frame = CGRectMake(ScreenWidth/5.0*3, 0, ScreenWidth/5.0, 40);
    button4.tag = kOperatorBtnTag_Control;
    [button4 setImage:[UIImage imageNamed:@"ic_operator_item_control.png"] forState:UIControlStateNormal];
    [button4 addTarget:self action:@selector(contactButtonAction:) forControlEvents:UIControlEventTouchUpInside];
    [self.midToolHView addSubview:button4];
     */
    
    //
    /*
    LCMonitorButton *button5 = [LCMonitorButton buttonWithType:UIButtonTypeCustom];
    button5.frame = CGRectMake(ScreenWidth/5.0*4, 0, ScreenWidth/5.0, 40);
    button5.tag = kOperatorBtnTag_Modify;
    [button5 setImage:[UIImage imageNamed:@"ic_operator_item_modify.png"] forState:UIControlStateNormal];
    [button5 addTarget:self action:@selector(contactButtonAction:) forControlEvents:UIControlEventTouchUpInside];
    [self.midToolHView addSubview:button5];
     */
...
{
    SDWebImageRootViewController *vc = [[SDWebImageRootViewController alloc] init];
...
- (void)contactButtonAction:(UIButton *)sender
{
    switch(sender.tag){
        case kOperatorBtnTag_Modify:
        {
            ...
            addContactNextController.contactId = self.monitorContact.contactId;
            addContactNextController.modifyContact = self.monitorContact;
        ...
        case kOperatorBtnTag_Message:
        {
            ...
            chatController.contact = self.monitorContact;
        ...
        case kOperatorBtnTag_Monitor:
        {
            ...
            [mainController setUpCallWithId:self.monitorContact.contactId password:self.monitorContact.contactPassword callType:P2PCALL_TYPE_MONITOR];
        }
            break;
        case kOperatorBtnTag_Chat:
        {
            MainController *mainController = [AppDelegate sharedDefault].mainController;
            [mainController setUpCallWithId:self.monitorContact.contactId password:@"0" callType:P2PCALL_TYPE_VIDEO];
        }
            break;
        case kOperatorBtnTag_Playback:
        {
            if (self.monitorContact.defenceState==DEFENCE_STATE_NO_PERMISSION) {
                ...
                playbackController.contact = self.monitorContact;
        ...
        case kOperatorBtnTag_Control:
        {
            ...
            //[[P2PClient sharedClient]getNpcSettingsWithId:self.monitorContact.contactId password:self.monitorContact.contactPassword];
        ...
        case kOperatorBtnTag_WeakPwd:
        {
            ...
            modifyDevicePasswordController.contact = self.monitorContact;
        ...
        {
            //设备检查更新
            if(self.monitorContact.result_sd_server==1){
                //读取到了服务器升级文件
                NSString *title = [NSString stringWithFormat:@"%@:%@,%@:%@",CustomLocalizedString(@"cur_version_is", nil),self.monitorContact.deviceCurVersion,CustomLocalizedString(@"can_update_to", nil),self.monitorContact.deviceUpgVersion];
            ...
            if(self.monitorContact.result_sd_server==72){
                //读取到了sd卡升级文件
                NSString *title = [NSString stringWithFormat:@"%@:%@,%@",CustomLocalizedString(@"cur_version_is", nil),self.monitorContact.deviceCurVersion,CustomLocalizedString(@"can_update_sd", nil)];
                UIAlertView *deleteAlert = [[UIAlertView alloc] initWithTitle:title message:@"" delegate:self cancelButtonTitle:CustomLocalizedString(@"cancel", nil) otherButtonTitles:CustomLocalizedString(@"ok", nil),nil];
                deleteAlert.tag = ALERT_TAG_UPDATE;
                [deleteAlert show];
                [deleteAlert release];
        ...
        case kOperatorBtnTag_initDevicePwd:
        {
            ...
            createInitPwdCtl.contactId = self.monitorContact.contactId;
...
-(void)onVerticalBtnPress:(UIButton *)button{
    switch(button.tag){
        case SOUND_BUTTON_H_TAG://声音
        {
            ...
            LCMonitorButton *button1 = [self.view viewWithTag:SOUND_BUTTON_H_TAG];
            BOOL isMute = [[PAIOUnit sharedUnit] muteAudio];
            if(isMute){
                [[PAIOUnit sharedUnit] setMuteAudio:NO];
                [button1 setImage:[UIImage imageNamed:@"喇叭-"] forState:UIControlStateNormal];
                
                //横屏，声音打开
                UIButton *controllerSoundBtn = (UIButton *)[self.controllBar viewWithTag:CONTROLLER_BTN_TAG_SOUND];
                [controllerSoundBtn setBackgroundImage:[UIImage imageNamed:@"喇叭关闭-"] forState:UIControlStateNormal];
            }else{
                
                [[PAIOUnit sharedUnit] setMuteAudio:YES];
                [button1 setImage:[UIImage imageNamed:@"monitor_sound_off_h.png"] forState:UIControlStateNormal];
...
-(void)btnClickToBack:(UIButton *)button{
    ...
    AppDelegate * tempDelegate = (AppDelegate *)[[UIApplication sharedApplication] delegate];
    MyFacilityViewController *mainVC = tempDelegate.mainVC;
    mainVC.isShowingMonitorController = NO;
}
...
- (void)backClick
{
    ...
    //相当于原项目OpenGLView中dealloc方法
    [self.remoteView setCaptureFinishScreen:YES];
    [self.remoteView vcDisappear];
}
...
//                soundImageViewH.image = [UIImage imageNamed:@"monitor_sound_on_h.png"];
                ...
                [button1 setImage:[UIImage imageNamed:@"喇叭关闭-"] forState:UIControlStateNormal];
...
//                soundImageViewH.image = [UIImage imageNamed:@"monitor_sound_off_h.png"];
        ...
        {
            UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:CustomLocalizedString(@"door_bell", nil) message:CustomLocalizedString(@"confirm_open", nil) delegate:self cancelButtonTitle:CustomLocalizedString(@"cancel", nil) otherButtonTitles:CustomLocalizedString(@"ok", nil), nil];
            [alertView show];
            [alertView release];
...
-(void)updateRightButtonState:(NSInteger)tag{
    ...
    if (tag == CONTROLLER_BTN_TAG_HD) {
        ...
        label.textColor = XBlue;
        [rButton setTitle:CustomLocalizedString(@"HD", nil) forState:UIControlStateNormal];
    }else if(tag == CONTROLLER_BTN_TAG_SD){
        UILabel *label = (UILabel *)[button viewWithTag:CONTROLLER_LABEL_TAG_SD];
        label.textColor = XBlue;
        [rButton setTitle:CustomLocalizedString(@"SD", nil) forState:UIControlStateNormal];
    }else if (tag == CONTROLLER_BTN_TAG_LD){
        UILabel *label = (UILabel *)[button viewWithTag:CONTROLLER_LABEL_TAG_LD];
        label.textColor = XBlue;
        [rButton setTitle:CustomLocalizedString(@"LD", nil) forState:UIControlStateNormal];
    }
