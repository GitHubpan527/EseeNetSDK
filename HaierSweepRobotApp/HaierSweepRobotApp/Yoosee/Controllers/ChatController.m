...
-(void)initComponent{
    ...
    self.navigationItem.title = self.contact.contactName;
    self.navigationItem.rightBarButtonItem = [UIBarButtonItem lc_itemWithIcon:@"ic_bar_btn_clear.png" block:^{
        [self onClearPress];
    }];

    
    UITableView *tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, width, height-BOTTOM_HEIGHT) style:UITableViewStylePlain];
    ...
    messageField.placeholder = CustomLocalizedString(@"input_message", nil);
    ...
    sendLabel.text = CustomLocalizedString(@"send", nil);
...
-(void)onSendButtonPress:(UIButton*)button{
    ...
    if(!messageStr||messageStr.length==0){
        [self.view makeToast:CustomLocalizedString(@"input_message", nil)];
        return;
    }
    
    if(messageStr.length>1024){
        [self.view makeToast:CustomLocalizedString(@"message_too_long", nil)];
        return;
    }
    LoginResult *loginResult = [UDManager getLoginInfo];
    
    if([self.contact.contactId isEqualToString:loginResult.contactId]){
        [self.view makeToast:CustomLocalizedString(@"can_not_send_message_to_myself", nil)];
        return;
    }
    ...
    if(result==-1){
        [self.view makeToast:CustomLocalizedString(@"send_failure", nil)];
        return;
    }
