        ...
        [[NetManager sharedManager] getAlarmRecordWithUsername:login.contactId  sessionId:login.sessionId option:@"1" msgIndex:index senderList:@"1069721" checkLevelType:@"1" vKey:@"123" callBack:^(id JSON) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                if (error_code == NET_RET_NO_RECORD) {
                    //无记录,显示提示信息
                    [self.view makeToast:CustomLocalizedString(@"no_more_record", nil)];
                    ...
                    [self.view makeToast:CustomLocalizedString(@"unknown_error", nil)];
                }else if (error_code == NET_RET_SYSTEM_MAINTENANCE_ERROR){
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        ...
        self.headlineArr = [NSMutableArray arrayWithObjects:CustomLocalizedString(@"alarm_message",nil),CustomLocalizedString(@"system_message",nil),nil];
    ...
    if (isLocalAlarmRecord) {
        ...
        [[NetManager sharedManager] getAlarmRecordWithUsername:login.contactId  sessionId:login.sessionId option:@"2" msgIndex:nil senderList:@"1069721" checkLevelType:@"1" vKey:@"123" callBack:^(id JSON) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                if (error_code == NET_RET_NO_RECORD) {
                    //无记录,显示提示信息
                    [self.view makeToast:CustomLocalizedString(@"no_record", nil)];
                }else if (error_code == NET_RET_NO_PERMISSION){
                    [self.view makeToast:CustomLocalizedString(@"no_permission", nil)];
                }else if (error_code == NET_RET_UNKNOWN_ERROR){
                    [self.view makeToast:CustomLocalizedString(@"unknown_error", nil)];
...
-(void)getRecommendInfoList{
    ...
    if (!storeID) {
        ...
        [self.view makeToast:CustomLocalizedString(@"mall_id_unexist", nil)];
        return ;
    }
    ...
    [NSURLConnection sendAsynchronousRequest:request queue:[NSOperationQueue mainQueue ] completionHandler:^(NSURLResponse *response, NSData *data, NSError *error) {
        ...
        if ((!error)) {//发送请求成功
            ...
            if (error_code == 0) {//成功获取到数据
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    [self.view makeToast:CustomLocalizedString(@"get_data_fail", nil)];
                });
            }
        }else{//发送请求失败
            dispatch_async(dispatch_get_main_queue(), ^{
                [self.view makeToast:CustomLocalizedString(@"send_request_fail", nil)];
            });
        }
    }];
}
...
-(void)initComponent{
    ...
    _height = rect.size.height-64;
    
    //navigation bar
//    self.navigationItem.title = @"摄像头消息";
    self.navigationItem.title = CustomLocalizedString(@"news_cameras",nil);
    
    if (isLocalAlarmRecord) {//显示清除本地报警记录按钮
        self.navigationItem.rightBarButtonItem = [UIBarButtonItem lc_itemWithTitle:CustomLocalizedString(@"delete",nil) block:^{
            [self clearPress];
        }];
    }else{
...
//    [topBar setTitle:CustomLocalizedString(@"message_item",nil)];
//    if (isLocalAlarmRecord) {//显示清除本地报警记录按钮
...
//    [topBar release];
    ...
    //headline
    UIView *headView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, _width, HEADVIEW_HEIGHT)];
    ...
    //
    UIScrollView* bottomScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, _width, _height)];
    bottomScrollView.contentSize = CGSizeMake(_width*1, _height);
...
-(void)clearPress{
    UIAlertView *deleteAlert = [[UIAlertView alloc] initWithTitle:CustomLocalizedString(@"sure_to_clear", nil) message:@"" delegate:self cancelButtonTitle:CustomLocalizedString(@"cancel", nil) otherButtonTitles:CustomLocalizedString(@"ok", nil),nil];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (tableView == self.systemMsgTableView) {
        ...
        if ([webViewController.imgURLLinkString isEqualToString:@""]) {
            [self.view makeToast:CustomLocalizedString(@"invalid_link", nil)];
