...
-(void)initComponent{
    ...
    //设备id
    NSString* textDevice = [NSString stringWithFormat:@"%@ %@",CustomLocalizedString(@"alarm_device", nil),self.contactId];
    ...
    //报警类型。。。
    NSString* alarmTest = CustomLocalizedString(@"somebody_visit", nil);
...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
    ...
    if(buttonIndex==1){//确定
        ...
        if(!inputPwd||inputPwd.length==0){
            [appDdelegate.mainController.view makeToast:CustomLocalizedString(@"input_device_password", nil)];
        }
        else
        {
