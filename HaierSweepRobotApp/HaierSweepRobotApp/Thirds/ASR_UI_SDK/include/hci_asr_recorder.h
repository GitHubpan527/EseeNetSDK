...
extern "C"
{
	...
	*/
    ...
    typedef enum
    {
        RECORDER_STATE_NOT_INIT,			///< 没有初始化
        RECORDER_STATE_IDLE,				///< 空闲状态，可以开始录音和识别
        RECORDER_STATE_RECORDING,			///< 正在录音
        RECORDER_STATE_RECOGING,			///< 正在识别
        RECORDER_STATE_CONFIRMING,			///< 正在提交用户确认结果
        RECORDER_STATE_ERROR,				///< 错误状态，此时录音机已经无法正常工作
     ...
     * @param    nVolume         录音音量大小,取值[0 ~ 100] 0~60表示静音，100声音最大
     * @param	pUsrParam		用户自定义参数
     */
    ...
    typedef struct _RECORDER_CALLBACK_PARAM {
        ...
        Callback_RecorderEventError pfnError;                   ///< 录音机识别中间结果回调
        void *pvErrorUsrParam;                                  ///< 录音机识别中间结果回调自定义参数
        Callback_RecorderEventRecogProcess pfnRecogProcess;     ///< 录音机错误回调
        void *pvRecogProcessParam;                              ///< 录音机错误回调自定义参数
     ...
     *		<tr><td>@ref RECORDER_ERR_OUT_OF_MEMORY</td><td>分配空间失败</td></tr>
     *	</table>
     */ 
