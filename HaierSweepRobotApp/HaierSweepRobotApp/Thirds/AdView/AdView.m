...
- (void)setimageToURL:(NSArray *)imageToURL{
    _toArray = imageToURL;
}
...
- (void)setimageLinkURL:(NSArray *)imageLinkURL
{
    ...
    {//只有一张图片
        ...
        //ljp
        [_adScrollView addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(rightTap)]];
    }
...
#pragma mark - 仅有一张图片 点击事件 ljp
-(void)rightTap
{
    if (_callBack) {
        _callBack(0,_toArray[0]);
    }
}
...
{
    if (_callBack) {
        _callBack(centerImageIndex,_toArray[centerImageIndex]);
    }
}
